...
const Home = {
  ...
  computed: {
    ...
    getLikeCookie(){
      // on recupere le Json et on le parse pour le JS 
  ...
  methods: {
    setLikeCookie(){
      document.addEventListener('input', () => {
        setTimeout(() => {                                      // on ajoute un timeout pour laisse liked s'incrimenter
          $cookies.set('like', JSON.stringify(this.liked));     //methode pour transformer tout les like en Json
        }, 300);
      })
    },
    addToCart(product){
    ...
    cartRemoveItem(id){
      this.$delete(this.cart, id)   //le delete est un élement vue
    }
  },
  mounted: () => {
    this.getLikeCookie;
